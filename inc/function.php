				...
				{			
					if(!empty($row['url'])){	
						if(!empty($navPer)&&in_array($row['id'],array_keys($navPer))){
							...
							for($i=0;$i<count($navPerId);$i++){	
                                  if(in_array($navPerId[$i]['id'],$pemidArr)){							
								    array_push($child_arr,array('id'=>$navPerId[$i]['id'],"state"=>array("opened"=>"boolean","selected"=>"boolean"),'showname'=>$navPerId[$i]['name'],'cateid'=>$navPerId[$i]['cateid'],'about'=>$navPerId[$i]['about'],'text'=>'['.$navPerId[$i]['id'].']'.$navPerId[$i]['name'],'nodetype'=>'pemid',"icon"=>"fa fa-cube icon-state-danger"));
								  }else{
									array_push($child_arr,array('id'=>$navPerId[$i]['id'],'showname'=>$navPerId[$i]['name'],'cateid'=>$navPerId[$i]['cateid'],'about'=>$navPerId[$i]['about'],'text'=>'['.$navPerId[$i]['id'].']'.$navPerId[$i]['name'],'nodetype'=>'pemid',"icon"=>"fa fa-cube icon-state-danger"));
								   } 
								 }
						    //if(in_array($row['id'],$treeNodeidArr)){
							//	array_push($tree_arr ,array("id"=>$row['id'],'text'=>$row['showname'],"state"=>array("opened"=>"boolean","selected"=>"boolean"),'nodetype'=>'page',"icon"=>"fa fa-file icon-state-warning",'children'=>$child_arr) );
